...
def generate_data():
    ...
    t = np.arange(0, 365 * 6)
    # 转换为日期
    dates = pd.date_range(start='2023-01-01', periods=365 * 6, freq='D')

    # 体长相关参数，调小生长系数 k 让生长速度变慢
    L_inf_length = 100  # 最大理论体长（厘米）
    k_length = 0.002  # 体长生长系数，调小该值减缓生长速度
    ...
    # 为体长和体重添加抖动
    length_jitter = np.random.normal(0, 0.7, lengths.shape)
    weight_jitter = np.random.normal(0, 100, weights.shape)
...
def main():
    ...
    # 设置标题字体大小
    plt.title('龙利鱼体重与体长预测', fontsize=16)
    plt.show()
