...
def generate_data():
    ...
    t = np.arange(0, 365 * 3)
    # 转换为日期
    dates = pd.date_range(start='2022-09-01', periods=365 * 3, freq='D')

    # 体长相关参数，调小生长系数 k 让生长速度变慢
    L_inf_length = 60  # 最大理论体长（厘米）
    ...
    weight_jitter = np.random.normal(0, 30, weights.shape)
...
def main():
    ...
    # 设置标题字体大小
    plt.title('海鲈鱼体重与体长预测', fontsize=16)
    plt.show()
